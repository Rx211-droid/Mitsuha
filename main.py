...
- New Fun Commands: /ping, /slap, /couple
"""
...
COUPLE_PHOTO_ID = "AgACAgUAAxkBAAId5GjLxQv_BxOm3_RGmB9j4WceUFg7AALdyzEb-tJgVuOn7v3_BWvqAQADAgADeQADNgQ"
...
async def callback_verify(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # lift restrictions
    try:
        # FIXED: Using a more compatible set of permissions to avoid errors on older library versions.
        await context.bot.restrict_chat_member(
            chat_id, user_id,
...
async def cmd_unmute(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.reply_to_message:
        ...
        return
    try:
        # FIXED: The original code had a potential error here on older library versions.
        # This new set of permissions is more compatible and restores main chat functions.
        await context.bot.restrict_chat_member(
            update.effective_chat.id, target.id,
            permissions=ChatPermissions(can_send_messages=True, can_send_polls=True,
                                        can_send_other_messages=True, can_add_web_page_previews=True)
